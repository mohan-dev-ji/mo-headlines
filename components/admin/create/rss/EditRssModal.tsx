...
interface EditRssModalProps {
  isOpen: boolean
  onClose: () => void
  producer: Producer | null
}

export function EditRssModal({ isOpen, onClose, producer }: EditRssModalProps) {
  const categories = useQuery(api.categories.getAllCategories)
  const updateProducer = useMutation(api.createRss.updateRssSource)
  const testFeed = useAction(api.createRss.testAndUpdateRssSource)
  ...
  const handleSubmit = async (data: ProducerFormData) => {
    ...
    try {
      ...
      // Test the feed after update
      await testFeed({
        sourceId: producer._id,
        feedUrl: data.feedUrl,
