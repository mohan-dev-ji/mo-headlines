...
export function CreateResearchModal({ isOpen, onClose }: CreateResearchModalProps) {
  const { user } = useUser()
  const categories = useQuery(api.categories.getAllCategories)
  const createResearchSource = useMutation(api.createResearch.createResearchSource)
  ...
  const handleSubmit = async (data: ResearchFormData) => {
    if (!user) {
      setSubmitError('You must be logged in to create research sources')
      return
    }
    ...
    try {
      ...
      setSubmitError(error instanceof Error ? error.message : 'Failed to create research source')
    } finally {
