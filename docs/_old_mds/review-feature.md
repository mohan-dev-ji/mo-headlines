# Article Management: Approve, Edit, Reject and Create Articles
...
- Tabs (backed by `articles.status`):
  ...
  - Create tab shows `status = "drafts`.
- Review:
...
- Create:
  ...
  - Uses similar dialogue to the edit article dialogue
- Bulk actions:
...
- Status transitions:
  ...
  - Admin sets to `approved`, `rejected` or `draft`.
